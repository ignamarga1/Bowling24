...
public class AprendizajeBolos : MonoBehaviour
{
    ...
    IEnumerator Entrenamiento()
    {
        ...
        if (casosEntrenamiento.numInstances() < 10)
        {
            ...
            for (float Fx = 1; Fx <= valorMaximoFx; Fx += paso)                             // Bucle de planificación de la fuerza FX durante el entrenamiento
            {
                for (float Fy = 1; Fy <= valorMaximoFy; Fy += paso)                         // Bucle de planificación de la fuerza FY durante el entrenamiento
                {
                    ...
                    print("ENTRENAMIENTO: con fuerza Fx " + Fx + " y Fy = " + Fy + " se alcanzó una distancia de " + rb.transform.position.x + " m"); // CAMBIAR por número de bolos?
        ...
        print(casosEntrenamiento.numInstances() + " espera " + saberPredecirDistancia.toString());
    ...
    void FixedUpdate()
    {
        if ((ESTADO == "Con conocimiento") && (distanciaObjetivo > 0))
        {
            ...
            // Si usa dos bucles Fx y Fy con "modelo físico aproximado", complejidad n^2
            // Reduce la complejidad con un solo bucle FOR, así
            for (float Fy = 1; Fy < valorMaximoFy; Fy = Fy + paso)                                              // Bucle FOR con fuerza Fy, deduce Fx = f(Fy, distancia) y escoge mejor combinación         
            {
                ...
                if ((Fx >= 1) && (Fx <= valorMaximoFx))
                {
                    ...
                    casoPrueba2.setValue(2, distanciaObjetivo);                                                 // y la distancia
                    float prediccionDistancia = (float)saberPredecirDistancia.classifyInstance(casoPrueba2);    // Predice Fx a partir de la distancia y una Fy
                    if ((Fx >= 1) && (Fx <= valorMaximoFx))
                    {
