...
public class AprendizLento_2_incognitas : MonoBehaviour
{
    ...
    float mejorFuerzaZ, mejorFuerzaY, distanciaObjetivo;
    ...
    IEnumerator Entrenamiento()
    {
        ...
        {
            texto.text = "ENTRENAMIENTO: crea una tabla con las fuerzas Fz y Fy utilizadas y las distancias alcanzadas.";
            ...
            for (float Fz = 1; Fz <= valorMaximoFz; Fz += paso)                             // Bucle de planificación de la fuerza FZ durante el entrenamiento
            {
                for (float Fy = 1; Fy <= valorMaximoFy; Fy = Fy + paso)                     // Bucle de planificación de la fuerza FY durante el entrenamiento
                {
                    ...
                    rb.AddForce(new Vector3(0, Fy, Fz), ForceMode.Impulse);                 // y la lanza con las fuerza Fz y Fy
                    ...
                    print("ENTRENAMIENTO: con fuerza Fz " + Fz + " y Fy = " + Fy + " se alcanzó una distancia de " + rb.transform.position.x + " m"); // CAMBIAR por número de bolos?
                    casoAaprender.setDataset(casosEntrenamiento);                           // Crea un registro de experiencia
                    casoAaprender.setValue(0, Fz);                                          // Guarda los datos de las fuerzas Fz y Fy utilizadas
        ...
        saberPredecirFuerzaX.buildClassifier(casosEntrenamiento);                        // REALIZA EL APRENDIZAJE DE FZ A PARTIR DE LA DISTANCIA Y FY

        saberPredecirDistancia = new M5P();                                              // Crea otro algoritmo de aprendizaje M5P (árboles de regresión)  
        casosEntrenamiento.setClassIndex(2);                                             // La variable a aprender a calcular la distancia dada FZ e FY                                                                                         
    ...
    // DURANTEL EL JUEGO: aplica lo aprendido para lanzar los bolos
    void FixedUpdate()                                                                                 
    {
        if ((ESTADO == "Con conocimiento") && (distanciaObjetivo > 0))
        {
            ...
            for (float Fy = 1; Fy < valorMaximoFy; Fy = Fy + paso)                                              // Bucle FOR con fuerza Fy, deduce Fz = f(Fy, distancia) y escoge mejor combinación         
            {
                ...
                float Fx = (float)saberPredecirFuerzaX.classifyInstance(casoPrueba);                            // Predice Fz a partir de la distancia y una Fy 
                if ((Fx >= 1) && (Fx <= valorMaximoFz))
                {
                    ...
                    casoPrueba2.setValue(0, Fx);                                                                // Crea una registro con una Fz
                    casoPrueba2.setValue(1, Fy);                                                                // Crea una registro con una Fy
                    float prediccionDistancia = (float)saberPredecirDistancia.classifyInstance(casoPrueba2);    // Predice la distancia dada Fz y Fy
                    if (Mathf.Abs(prediccionDistancia - distanciaObjetivo) < menorDistancia)                    // Busca la Fy con una distancia más cercana al objetivo
                    {
                        ...
                        mejorFuerzaZ = Fx;                                                                       // de la fuerzas que uso, Fx
                        mejorFuerzaY = Fy;                                                                       // también Fy
                        print("RAZONAMIENTO: Una posible acción es ejercer una fuerza Fz = " + mejorFuerzaZ + " y Fy= " + mejorFuerzaY + " se alcanzaría una distancia de " + prediccionDistancia);
                    }
                }
            }                                                                                                    // FIN DEL RAZONAMIENTO PREVIO
            
            if ((mejorFuerzaZ == 0) && (mejorFuerzaY == 0)) { 
            ...
            else
            {
                ...
                r.AddForce(new Vector3(mejorFuerzaZ, mejorFuerzaY, 0), ForceMode.Impulse);                              // la lanza en el videojuego con la fuerza encontrada
                print("DECISION REALIZADA: Se lanzó bola con fuerza Fz = " + mejorFuerzaZ + " y Fy = " + mejorFuerzaY);
        ...
        {
            texto.text = "Para una canasta a " + distanciaObjetivo.ToString("0.000") + " m, las fuerzas Fz y Fy a utilizar será: " + mejorFuerzaZ.ToString("0.000") + "N y " + mejorFuerzaY.ToString("0.000") + "N, respectivamente";
            ...
            {                                                                          // Escribe la distancia en x alcanzada
                ...
                print("La bola lanzada llegó a " + r.transform.position.x + ". El error fue de " + (r.transform.position.x - distanciaObjetivo).ToString("0.000000") + " m");
                r.isKinematic = true;
                ESTADO = "FIN";
            }
        }
